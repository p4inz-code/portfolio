...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    body: [
      "Home: the hero now reads I identify unmet needs in software and turn them into products. Its words light up as you scroll, then a three-step line (spot the gap, build it, ship it) fills while the AP logo travels along it. A proof strip counts up four plain facts, and while the project chapters scroll a glass ladder on the left edge shows where you are, with the logo sliding from dot to dot. Click a dot and it jumps straight to that chapter. On screens without spare side space it becomes a small pill under the header, so it never sits on top of text.",
      "The browser tab icon is now the AP mark inside a ring that fills as you scroll. It stays still with reduced motion or when motion is paused.",
      "Work: /work is now a stacked card deck plus an index of every project, so it no longer scrolls like the home page. AniFX Fest 2026 and Kalasadhana Academy are listed. They are websites, so they are not counted in the products figure.",
      "Scenes: Nexus, Kanvaz, Ascent, Mission OS and Pursue OS each have their own backdrop and effects, driven by scroll on every device. Ascent stars are visible in the light theme, and there is a pause-motion button for ambient effects.",
      "Cards on every page are frosted glass in both themes. Contact drops the magnet card for a live composer with a clock, copy-email and a send animation.",
      "New /support page with Buy Me a Coffee and UPI, and a footer link. Every public repo now carries the Buy Me a Coffee link in its README support block and a FUNDING file. The button is a plain link rather than an embedded widget, so the site still loads nothing from third parties.",
      "Slashes used as separators in visible text are gone, and the 2 of 5 style counters use a bar instead.",
      "Checks: 8 pages at 18 widths from 320 to 2560 with no horizontal overflow, Lighthouse accessibility 100 on mobile. Not tested on a real iPhone, Android device or Firefox.",
    ],
  },
  {
    version: 'v5.24.1',
  ...
  },
  {
    ...
    body: [
      ...
      'Obscura gets a genuinely bespoke card instead of the shared ProjectCard template — leads with the real all 340 test ledger and a filled progress bar instead of a tagline, on both the homepage and /work.',
  ...
  },
  {
    ...
    body: [
      ...
      'HIGH — Obscura was listed as v0.9.0 with "ongoing feature development toward v1.0" — user had already shipped v1.0.0 with all 340 tests. projects.ts bumped.',
